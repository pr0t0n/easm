...
AGENT_RECON_SUBDOMAIN = AgentManifest(
    ...
    tools=["subfinder", "amass", "amass-brute", "amass-intel", "sublist3r", "findomain", "dnsx", "shuffledns", "assetfinder", "alterx", "dnsrecon-brt", "dnsrecon-zt", "dnsenum"],
...
AGENT_RECON_WEB = AgentManifest(
    ...
    tools=["katana", "hakrawler", "gau", "waybackurls", "gospider", "arjun", "paramspider", "ffuf-params", "wfuzz"],
    required_skills=["web_crawl", "js_analysis", "parameter_discovery", "parameter_name_fuzzing"],
    when_to_activate="Map web surface, extract JavaScript, find hidden endpoints and fuzz candidate parameter names.",
    phase_ids=["P03", "P04"],
    priority=8,
    timeout_seconds=900,
)
...
AGENT_RECON_FINGERPRINT = AgentManifest(
    ...
    name="HTTP Headers & TLS Fingerprint Agent",
    category="reconnaissance",
    description="OWASP Top 10 security-header analysis, HTTP/TLS metadata, WAF detection and tech stack identification.",
    tools=["httpx", "whatweb", "nikto", "curl-headers", "sslscan", "wafw00f"],
    required_skills=["http_fingerprint", "owasp_header_analysis", "tls_analysis", "waf_detection"],
    when_to_activate="Characterize tech stack, HTTP header posture, TLS hints and defense posture.",
    phase_ids=["P05", "P06"],
    priority=8,
    timeout_seconds=600,
)
...
AGENT_VULN_AUTH = AgentManifest(
    ...
    category="vulnerability",
    description="Credential fuzzing, JWT analysis, OAuth/MFA bypass and controlled brute-force through Kali Hydra profiles.",
    tools=["hydra", "medusa", "crackmapexec", "jwt_tool", "nuclei", "curl-headers"],
    required_skills=["auth_testing", "jwt_analysis", "hydra_userlist_passlist", "credential_fuzzing", "rate_limit_detection", "brute_force_safety"],
    when_to_activate="Validate login endpoints, tokens and session management only when authorization and credential lists are explicit.",
    phase_ids=["P14"],
    priority=8,
...
AGENT_VULN_DIRECTORY = AgentManifest(
    ...
    category="vulnerability",
    description="Fuzzing for hidden directories, files, backup artifacts, admin panels, parameters and form fields using Kali Seclists paths.",
    tools=["ffuf", "ffuf-files", "ffuf-params", "ffuf-values", "ffuf-post", "wfuzz", "gobuster", "feroxbuster", "dirsearch"],
    required_skills=["path_enumeration", "seclists_wordlist_selection", "ffuf_fuzzing", "parameter_fuzzing", "form_fuzzing", "rate_limiting"],
    when_to_activate="Enumerate directories/files and fuzz URL/form variables on each discovered host before exploitation.",
    phase_ids=["P15"],
    priority=8,
    timeout_seconds=900,
)
...
AGENT_VULN_API = AgentManifest(
    ...
    required_skills=["api_testing", "endpoint_discovery", "parameter_fuzzing", "post_body_fuzzing", "rate_limit_bypass"],
    when_to_activate="Test API endpoints, parameters and request bodies for common flaws and authentication gaps.",
    phase_ids=["P16"],
    priority=8,
    timeout_seconds=900,
)
...
AGENT_VULN_SSL = AgentManifest(
    ...
    category="vulnerability",
    description="Certificate chain, expiry, trust, protocol weakness and cipher suite analysis.",
    tools=["sslscan", "testssl", "nmap", "curl-headers"],
    required_skills=["ssl_analysis", "cipher_grading", "cert_validation", "certificate_chain_review", "hsts_review"],
    when_to_activate="Audit HTTPS endpoints for TLS configuration, certificate chain, expiry and downgrade risks.",
    phase_ids=["P18"],
    priority=7,
    timeout_seconds=600,
)
...
AGENT_VULN_IDOR = AgentManifest(
    ...
    category="vulnerability",
    description="IDOR/BOLA and access-control validation with endpoint mapping, parameter discovery and two-identity comparison.",
