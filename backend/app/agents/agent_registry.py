...
AGENT_VULN_AUTH = AgentManifest(
    ...
    category="vulnerability",
    description="Credential checks, JWT analysis, OAuth/MFA bypass and controlled brute-force through Kali Hydra profiles.",
    tools=["hydra", "medusa", "crackmapexec", "jwt_tool", "nuclei", "curl-headers"],
    required_skills=["auth_testing", "jwt_analysis", "hydra_userlist_passlist", "rate_limit_detection", "brute_force_safety"],
...
AGENT_VULN_DIRECTORY = AgentManifest(
    ...
    category="vulnerability",
    description="Hidden directory/file discovery, backup files and admin panels using Kali Seclists paths.",
    tools=["ffuf", "gobuster", "feroxbuster", "dirsearch"],
    required_skills=["path_enumeration", "seclists_wordlist_selection", "ffuf_fuzzing", "rate_limiting"],
    when_to_activate="Enumerate web directories/files on each discovered host with raft-small-directories/files before exploitation.",
    phase_ids=["P15"],
    priority=8,
    timeout_seconds=900,
)
