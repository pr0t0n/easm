...
def get_verification_status(tool_name: str, finding: dict[str, Any]) -> str:
    ...
    # A 401/403 -> validation response transition proves that the request
    # reached a different layer, not that an unauthorized protected action
    # succeeded. Auth bypass therefore remains candidate until a dedicated
    # validator supplies a proof pack. SSRF is confirmed only by its OOB
    # callback result.
