    ...
    *,
    ...
    if not _quality_gate.get("completion_allowed"):
        ...
        if (
            ...
            state["quality_gate_hard_block_fingerprint"] = list(quality_gate_blocker_fingerprint(_quality_gate))
        ...
        # No generator exists that can act on these blockers (they never
        # populated `actions`), and either the retry budget is spent or
        # another retry is provably futile (identical blockers, no action
        # scheduled last round). Nothing will ever look at this scan again if
        # it is left "blocked" -- auto-complete with gaps recorded instead,
        # the same outcome POST /scans/{id}/finalize gives an operator who
