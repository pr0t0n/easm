...
BAS_TECHNIQUE_CATALOG: list[dict[str, Any]] = [
    ...
    _technique(
        ...
        category="cloud_identity", mode="intruder", risk_tier="safe",
        display_name="Azure AD / Entra ID Tenant Discovery",
        description="Queries public Microsoft identity discovery endpoints for tenant federation and managed-domain exposure signals.",
        supported_os=[], mitre_refs=["T1590.005", "T1087.004"],
        kali_tool_name="curl-clouddir-bas", kali_profile="cloud_directory_scouting_check",
        availability="simulated", execution_backend="kali_proxychains",
        requires_tcp=True, is_simulated_in_phase_1=True,
        target_format="domain",
        recommendation="Revise exposicao de federation metadata e tenant discovery; alinhe respostas publicas com a postura desejada para dominios corporativos.",
    ),
    _technique(
        "m365_tenant_exposure_check",
        category="saas", mode="intruder", risk_tier="safe",
        display_name="Microsoft 365 Tenant Exposure Check",
        description="Validates externally observable Microsoft 365/Entra tenant discovery signals for a corporate domain through the agent path.",
        supported_os=[], mitre_refs=["T1590.005", "T1087.004"],
        kali_tool_name="curl-clouddir-bas", kali_profile="cloud_directory_scouting_check",
        availability="simulated", execution_backend="kali_proxychains",
        requires_tcp=True, is_simulated_in_phase_1=True,
        target_format="domain",
        recommendation="Endureca configuracoes de tenant discovery, federation e acesso legado; confirme que politicas de Conditional Access cobrem apps M365 de alto risco.",
    ),
    _technique(
        "aws_iam_path_analysis",
        category="cloud_identity", mode="intruder", risk_tier="safe",
        display_name="AWS IAM Path Analysis",
        description="Analyzes IAM users, roles, trust policies and privilege paths from read-only AWS inventory data.",
        supported_os=[], mitre_refs=["T1078.004", "T1098"],
        kali_tool_name=None, kali_profile=None,
        availability="planned", execution_backend=None,
        target_format="domain",
        recommendation="Inventarie roles, trust policies e permissive paths; aplique least privilege, SCPs e alertas para AssumeRole incomum.",
    ),
    _technique(
        "google_workspace_exposure_check",
        category="saas", mode="intruder", risk_tier="safe",
        display_name="Google Workspace Exposure Check",
        description="Validates Google Workspace domain posture, externally visible identity configuration and risky sharing exposure from read-only admin data.",
        supported_os=[], mitre_refs=["T1078.004", "T1530"],
        kali_tool_name=None, kali_profile=None,
        availability="planned", execution_backend=None,
        target_format="domain",
        recommendation="Revise domain-wide delegation, app access control, compartilhamentos externos e politicas de MFA para grupos privilegiados.",
    ),
    _technique(
        "okta_misconfiguration_check",
        category="saas", mode="intruder", risk_tier="safe",
        display_name="Okta Misconfiguration Check",
        description="Validates Okta org policy, weak app assignments, factor enrollment gaps and risky integration settings from read-only Okta data.",
        supported_os=[], mitre_refs=["T1078.004", "T1556"],
        kali_tool_name=None, kali_profile=None,
        availability="planned", execution_backend=None,
        target_format="domain",
        recommendation="Aplique MFA obrigatorio por grupo/app, restrinja integrações OAuth/OIDC e monitore eventos de policy bypass e factor reset.",
    ),
    _technique(
        "mfa_bypass_simulation_safe",
        category="cloud_identity", mode="bypass", risk_tier="safe",
        display_name="Safe MFA Bypass Simulation",
        description="Simulates non-destructive MFA bypass preconditions such as legacy auth, weak factor coverage and missing step-up rules from identity policy data.",
        supported_os=[], mitre_refs=["T1110", "T1556"],
        kali_tool_name=None, kali_profile=None,
        availability="planned", execution_backend=None,
        target_format="domain",
        recommendation="Bloqueie legacy authentication, exija phishing-resistant MFA para admins e aplique step-up em apps sensiveis.",
    ),
    _technique(
        "token_abuse_simulation",
        category="cloud_identity", mode="bypass", risk_tier="elevated",
        display_name="Token Abuse Simulation",
        description="Validates token replay, stale refresh token and over-scoped OAuth app abuse paths using controlled, read-only identity test sessions.",
        supported_os=[], mitre_refs=["T1528", "T1550.001"],
        kali_tool_name=None, kali_profile=None,
        availability="planned", execution_backend=None,
        target_format="domain",
        recommendation="Reduza lifetime de tokens, monitore refresh-token reuse, restrinja OAuth consent e revogue apps com permissões excessivas.",
    ),
    _technique(
        "conditional_access_validation",
        category="cloud_identity", mode="bypass", risk_tier="safe",
        display_name="Conditional Access Validation",
        description="Validates whether Conditional Access policies block risky locations, unmanaged devices and sensitive SaaS apps using controlled policy telemetry.",
        supported_os=[], mitre_refs=["T1078.004", "T1556"],
        kali_tool_name=None, kali_profile=None,
        availability="planned", execution_backend=None,
        target_format="domain",
        recommendation="Crie politicas por risco, localidade, device compliance e app; teste exclusoes e break-glass accounts de forma periodica.",
    ),
    _technique(
        "impossible_travel_telemetry",
        category="cloud_identity", mode="intruder", risk_tier="safe",
        display_name="Impossible Travel / Suspicious Login Telemetry",
        description="Validates whether identity telemetry produces expected detections for impossible travel and suspicious login scenarios.",
        supported_os=[], mitre_refs=["T1078.004", "T1589"],
        kali_tool_name=None, kali_profile=None,
        availability="planned", execution_backend=None,
        target_format="domain",
        recommendation="Ative alertas de sign-in risk, impossible travel e unfamiliar properties; integre logs de IdP ao SIEM com playbooks de resposta.",
    ),
    _technique(
        "port_service_scan",
